...
transfer_function <- function(fasta_mt, fasta_pt, bed_mt, bed_pt, 
  ...
  # Definicje regex dla tRNA i rRNA
  trna_regex <- "^trn|tRNA"
  ...
  # Funkcja do scalania nakładających się regionów (rozwiązuje problem > 100%)
  merge_intervals <- function(starts, ends) {
    if (length(starts) == 0) return(0)
    ord <- order(starts)
    s <- starts[ord]; e <- ends[ord]
    m_start <- s[1]; m_end <- e[1]
    total <- 0
    if(length(s) > 1) {
      for (i in 2:length(s)) {
  ...
  # Adnotacja i obliczanie statystyk
  annotate_and_get_metrics <- function(df, bed, prefix) {
    c_col <- if(prefix == "q") "q_id_low" else "s_id_low"
    s_col <- if(prefix == "q") "q_start_fix" else "s_start_fix"
    e_col <- if(prefix == "q") "q_end_fix" else "s_end_fix"
    
    lapply(1:nrow(df), function(i) {
      hit <- df[i, ]
      overlaps <- bed[bed$chrom == hit[[c_col]] & bed$start < hit[[e_col]] & bed$end > hit[[s_col]], ]
      ...
      # Statystyki liczone tylko dla białek (jeśli są) lub dla nc (jeśli nie ma białek)
      stats_overlaps <- if(has_protein) overlaps[!is_nc, ] else overlaps
      
      rel_ov_starts <- pmax(stats_overlaps$start, hit[[s_col]])
      rel_ov_ends   <- pmin(stats_overlaps$end, hit[[e_col]])
      ...
      gene_results <- lapply(1:nrow(overlaps), function(j) {
        g_l <- overlaps$end[j] - overlaps$start[j]
        ov  <- max(0, min(overlaps$end[j], hit[[e_col]]) - max(overlaps$start[j], hit[[s_col]]))
        list(name = overlaps$name[j], g_len = g_l, ov_len = ov, g_p = (ov/g_l)*100, t_p = (ov/hit$alig_length)*100)
      })
      
      res_text <- sapply(gene_results, function(x) {
        paste0(x$name, " (g_len=", x$g_len, ", ov_len=", round(x$ov_len, 0), 
               ", g_perc=", round(x$g_p, 1), "%, t_perc=", round(x$t_p, 1), "%)")
      })
      
      return(list(text = paste(res_text, collapse = "; "), 
                  sum_g_perc = sum_g_perc, sum_t_perc = sum_t_perc, 
                  only_nc = all(is_nc), has_protein = has_protein))
    })
  }
  ...
  for (i in 1:nrow(blast_n)) {
    ...
    # 1. Bezwzględne pierwszeństwo białka nad tRNA/rRNA
    ...
    # 2. Jeśli obie strony mają białka LUB obie mają tylko ncRNA
    if ((m$has_protein && p$has_protein) || (m$only_nc && p$only_nc)) {
