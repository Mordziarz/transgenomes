...
transfer_function_nuclear <- function(fasta_mt, fasta_pt, fasta_nuc, 
  ...
  run_single_transfer <- function(q_fasta, s_fasta, q_bed, s_bed, q_label, s_label, 
    ...
    annotate_metrics <- function(df, bed, prefix) {
      ...
      lapply(1:nrow(df), function(i) {
        ...
        if (nrow(ov) == 0) {
          return(list(text = NA, sum_g = 0, sum_t = 0, has_genes = FALSE, only_rna = FALSE, only_trna = FALSE, only_rrna = FALSE))
        }
        ...
        gene_list <- lapply(1:nrow(target_ov), function(j) {
          ...
          c(g_perc = (ov_len / g_len) * 100, t_perc = (ov_len / hit$alig_length) * 100, ov_len = ov_len, g_len = g_len)
        })
        gene_mat <- do.call(rbind, gene_list)
        
        res_text <- paste(sapply(1:nrow(target_ov), function(j) {
          paste0(target_ov$name[j], " (g_len=", gene_mat[j, "g_len"], ", ov_len=", round(gene_mat[j, "ov_len"], 0),
                 ", g_perc=", round(gene_mat[j, "g_perc"], 1), "%, t_perc=", round(gene_mat[j, "t_perc"], 1), "%)")
        }), collapse = "; ")
        
        return(list(text = res_text, sum_g = sum(gene_mat[,"g_perc"]), sum_t = sum(gene_mat[,"t_perc"]),
                    has_genes = TRUE, only_rna = (all(is_rna) && any(is_rna)), 
    ...
    for (i in 1:nrow(blast_n)) {
      ...
      if (!is.null(nuc_validation_blast)) {
        ...
        }, error = function(e) warning(sprintf("Nuclear validation error at row %d: %s", i, e$message)))
      }
      ...
      if (!m$has_genes && !p$has_genes) { blast_n$direction[i] <- "Undefined (No genes)"; next }
      ...
      if (!m$only_rna && p$only_rna) { blast_n$direction[i] <- paste(q_label, "->", s_label); next }
      if (m$only_rna && !p$only_rna) { blast_n$direction[i] <- paste(s_label, "->", q_label); next }
  ...
  res_mt_pt  <- run_single_transfer(fasta_mt, fasta_pt, b_mt, b_pt, "MT", "PT", r_mt_nuc, "NUC", "mt", "pt", r_mt_nuc)
  res_mt_nuc <- run_single_transfer(fasta_mt, fasta_nuc, b_mt, b_nuc, "MT", "NUC", r_mt_pt, "PT", "mt", "nuc", NULL)
  res_pt_nuc <- run_single_transfer(fasta_pt, fasta_nuc, b_pt, b_nuc, "PT", "NUC", r_mt_pt, "MT", "pt", "nuc", NULL)
  ...
  order_cols <- c("query_id", "subject_id", "perc_identity", "alig_length", "q_start", "q_end", "s_start", "s_end", 
                  "evalue", "bit_score", "mt_genes", "pt_genes", "nuc_genes", "direction", "excluded_coords", "nuclear_paralog_flag")
