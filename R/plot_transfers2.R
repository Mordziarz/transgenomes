...
plot_transfers2 <- function(transfer_function_out, 
  ...
  data_proc <- data_clean %>%
    ...
    mutate(
      m_mid = scale_pos((mt_start + mt_end) / 2, mt_off, total_mt_axis, normalization),
      p_mid = scale_pos((pt_start + pt_end) / 2, pt_off, total_pt_axis, normalization),
  ...
  ggplot(data_proc) +
    ...
    geom_segment(data = mt_info, 
                 ...
                 linewidth = 3.5, color = mt_sector_col, lineend = "butt") +
    geom_segment(data = pt_info, 
                 ...
                 linewidth = 3.5, color = pt_sector_col, lineend = "butt") +
